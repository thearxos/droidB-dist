...
struct MvtHandle(Arc<Mutex<Option<u32>>>);
...
const ALLOWED_SUBCOMMANDS: &[&str] = &[
    ...
    "harden", "engines", "deeplinks", "mtk", "mvt", "clean",
];
...
/// A fleet spyware scan, streamed. Same shape as logcat_stream: a worker thread reads the
/// child's stdout/stderr into a channel and the async command drains it into app.emit(), so
/// 100 phones' worth of output reaches the UI as it happens instead of after the last one.
/// The scan is long by nature (each phone is copied off, then checked), which is exactly why
/// it streams and why it can be stopped.

/// The phones a scan could target, structured (never scraped from coloured CLI text).
#[tauri::command]
fn mvt_devices() -> Result<serde_json::Value, String> {
    let out = Command::new(droidb_bin()).args(["mvt", "devices", "--json"]).output()
        .map_err(|e| format!("failed to run droidB-native: {e}"))?;
    serde_json::from_slice(&out.stdout).map_err(|e| format!("bad JSON from mvt devices: {e}"))
}

/// Dependency state for the device-support card.
#[tauri::command]
fn mvt_setup() -> Result<ActionResult, String> { run_action(vec!["mvt".into(), "setup".into()]) }

#[tauri::command]
async fn mvt_scan_stream(app: tauri::AppHandle, state: tauri::State<'_, MvtHandle>, serial: Option<String>, workers: u32) -> Result<i32, String> {
    use tauri::Emitter;
    let mut args: Vec<String> = vec!["mvt".into(), "fleet".into(), "--workers".into(), workers.max(1).to_string()];
    if let Some(s) = serial.filter(|s| !s.trim().is_empty()) { args.push("--serial".into()); args.push(s); }
    let bin = droidb_bin();
    let (tx, rx) = std::sync::mpsc::channel::<String>();
    let pid_slot = state.0.clone();
    let handle = std::thread::spawn(move || -> i32 {
    ...
    for line in rx { let _ = app.emit("mvt-line", line); }
    handle.join().map_err(|_| "scan worker panicked".to_string())
}

#[tauri::command]
fn mvt_scan_stop(state: tauri::State<'_, MvtHandle>) -> Result<(), String> {
...
fn main() {
    tauri::Builder::default()
        ...
        .manage(MvtHandle(Arc::new(Mutex::new(None))))
        .invoke_handler(tauri::generate_handler![
            run_action,
            mvt_scan_stream, mvt_scan_stop, mvt_devices, mvt_setup,
