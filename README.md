...
One command, batteries included. It installs:
...
| udev + usbmuxd | `51-droidB-android.rules` for non-root device access, and usbmuxd enabled so an iPhone is seen on plug-in |

The two compiled pieces come from the [droidB-dist](https://github.com/thearxos/droidB-dist)
release and are checked against their published SHA-256 before install. If a download
fails the rest of the toolkit still works, so a dropped connection never leaves a
half-installed system. Re-run `install.sh` to pick up what was missed.
