...
/* ---------------- Spyware Scan (MVT) ----------------
   Devices come from `mvt devices --json`, never scraped from coloured CLI text.
   The scan streams: the backend emits one `mvt-line` per line as it happens, because a
   fleet scan copies each phone off before checking it and can run for many minutes. */
let mvtLoaded = false, mvtRunning = false;

function mvtClassify(t) {
  const l = t.toLowerCase();
  // Only the wrapper's structured-result summary establishes a match. Engine
  // prose such as "no malicious files detected" must never become a detection.
  if (/spyware indicators matched \([1-9]\d*\)/.test(l)) return 'detect';
  if (l.includes('error') || l.includes('failed') || l.includes('traceback')) return 'warn';
  if (l.includes('warning') || l.includes('alert') || l.includes('coverage:') || l.includes('asleep')) return 'warn';
  if (l.includes('no spyware indicators matched')) return 'info';
  if (l.startsWith('──') || l.includes('done ·')) return 'info';
  return 'step';
}

function mvtLog(text) {
  const box = $('#mvt-log');
  const empty = box.querySelector('.con-empty');
  if (empty) empty.remove();
  const clean = stripAnsi(text).replace(/\s+$/, '');
  if (!clean) return;
  const ln = document.createElement('span');
  ln.className = 'ln ln-' + mvtClassify(clean);
  ln.textContent = clean;
  box.appendChild(ln);
  box.scrollTop = box.scrollHeight;
}
function mvtBusy(on, text) {
  mvtRunning = on;
  $('#mvt-busy').hidden = !on;
  $('#mvt-stop').hidden = !on;
  if (text) $('#mvt-busy-text').textContent = text;
}

listen('mvt-line', e => mvtLog(String(e.payload)));

async function mvtLoadDevices() {
  const list = $('#mvt-fleet'), count = $('#mvt-count');
  try {
    const devs = await invoke('mvt_devices');
    list.innerHTML = '';
    count.textContent = devs.length === 0 ? 'No phones connected'
      : (devs.length === 1 ? '1 phone connected' : devs.length + ' phones connected');
    if (!devs.length) {
      list.innerHTML = '<div class="fl"><span class="nm" style="color:var(--dim)">Plug a phone in with USB debugging on, or connect an iPhone and trust this computer.</span></div>';
      return;
    }
    for (const d of devs) {
      const row = document.createElement('div');
      row.className = 'fl';
      row.innerHTML = '<span class="status-dot ok"></span>'
        + '<span class="nm"></span>'
        + '<span class="plat"></span>'
        + '<span class="sn"></span>'
        + '<button class="go">Scan</button>';
      row.querySelector('.nm').textContent = d.label;
      row.querySelector('.plat').textContent = d.platform === 'ios' ? 'iOS' : 'Android';
      row.querySelector('.sn').textContent = d.serial;
      row.querySelector('.go').addEventListener('click', () => mvtScan(d.serial, 1, d.label));
      list.appendChild(row);
    }
  } catch (e) {
    count.textContent = 'Could not list phones';
    list.innerHTML = '<div class="fl"><span class="nm" style="color:var(--bad)"></span></div>';
    list.querySelector('.nm').textContent = errText(e);
  }
}

async function mvtLoadStatus() {
  try {
    const r = await invoke('run_action', { args: ['mvt', 'status'] });
    $('#mvt-engine').innerHTML = '<pre class="mvt-sub" style="margin:0;white-space:pre-wrap;font:inherit"></pre>';
    $('#mvt-engine').querySelector('pre').textContent = stripAnsi(r.output);
  } catch (e) { $('#mvt-engine').textContent = errText(e); }
  try {
    const r = await invoke('mvt_setup');
    $('#mvt-support').innerHTML = '<pre class="mvt-sub" style="margin:0;white-space:pre-wrap;font:inherit"></pre>';
    $('#mvt-support').querySelector('pre').textContent = stripAnsi(r.output);
  } catch (e) { $('#mvt-support').textContent = errText(e); }
}

async function mvtScan(serial, workers, label) {
  if (mvtRunning) { alert('A scan is already running.'); return; }
  mvtBusy(true, serial ? ('Scanning ' + (label || serial) + '…') : 'Scanning every connected phone…');
  mvtLog(serial ? ('Starting scan of ' + (label || serial)) : 'Starting scan of every connected phone');
  try {
    const code = await invoke('mvt_scan_stream', { serial: serial || null, workers: Number(workers) || 4 });
    mvtLog(code === 0 ? 'Scan finished.' : ('Scan ended with code ' + code + '.'));
  } catch (e) {
    mvtLog('Scan failed: ' + errText(e));
  } finally {
    mvtBusy(false);
  }
}

$('#mvt-refresh')?.addEventListener('click', mvtLoadDevices);
$('#mvt-stop')?.addEventListener('click', async () => { await invoke('mvt_scan_stop').catch(() => {}); mvtLog('Stopping…'); });
$('#mvt-scan-all')?.addEventListener('click', () => mvtScan(null, $('#mvt-workers').value));
$('#mvt-iocs')?.addEventListener('click', async () => {
  mvtBusy(true, 'Updating fingerprints…');
  try { const r = await invoke('run_action', { args: ['mvt', 'iocs'] }); mvtLog(stripAnsi(r.output)); await mvtLoadStatus(); }
  catch (e) { mvtLog(errText(e)); } finally { mvtBusy(false); }
});
$('#mvt-setup')?.addEventListener('click', async () => {
  mvtBusy(true, 'Installing device support…');
  try { const r = await invoke('mvt_setup'); mvtLog(stripAnsi(r.output)); await mvtLoadStatus(); }
  catch (e) { mvtLog(errText(e)); } finally { mvtBusy(false); }
});
$('#mvt-scan-file')?.addEventListener('click', () => {
  alert('Pick a backup, bug report or acquisition folder from the command line for now:\n\n  droidB mvt bugreport <file.zip>\n  droidB mvt backup <file.ab>\n  droidB mvt ios-backup <folder>');
});

// load the panel the first time it is opened, not on every app start
$$('.nav-item').forEach(b => b.addEventListener('click', () => {
  if (b.dataset.panel === 'mvt' && !mvtLoaded) { mvtLoaded = true; mvtLoadDevices(); mvtLoadStatus(); }
}));
