...
fn main() {
    ...
    // Prefer the STATIC archive so the driver ends up inside the droidB binary.
    // Linking libodin4.so instead records a hard DT_NEEDED, and then droidB refuses
    // to even start on a machine without the library -- taking every unrelated
    // feature down with it, which is the exact failure the header comment warns
    // about. Static keeps droidB one self-contained file.
    let static_lib = lib_dir.join("libodin4_static.a");
    if static_lib.is_file() {
        println!("cargo:rustc-link-lib=static=odin4_static");
        // A C++ static archive carries no transitive deps of its own, so name them.
        println!("cargo:rustc-link-lib=dylib=stdc++");
        for dep in ["usb-1.0", "cryptopp", "lz4", "z"] {
    ...
    println!("cargo:warning=droidB-thor: only the shared libodin4 was found, so this droidB binary will REQUIRE it at runtime. Build droidB-thor's libodin4_static.a for a self-contained binary.");
